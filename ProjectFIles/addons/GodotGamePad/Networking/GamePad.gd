...
remote func _on_button_pressed(side, button):
	...
	var player_dict = players.get(id)
	
	if !player_dict.get(side):
		player_dict[side] = {}
	
	player_dict.get(side)[button] = true
...
remote func _on_button_released(side, button):
	...
	player_dict.get(side)[button] = false
...
remote func _on_input_direction_calculated(side, direction, intensity):
	...
	player_dict.get(side)["input_direction"] = direction
	player_dict.get(side)["input_intensity"] = intensity
